...
def semantic_search(question):
    ...
    context_lenght = context_lenght = len(df_context) -1
...
def summarize_question_with_history(chat_history, question):
    summary_prompt = f"""
                      ...
                      """
    summary = Complete(st.session_state.model, summary_prompt)
    return summary
...
def process_complete(question):
    ...
    prompt = f"""
              ...
              If you do not have the information providedin the CONTEXT just say so. 
    ...
    response = Complete(st.session_state.model, prompt) + f'\n For more info visit - {url_link}'
    return response
...
docs = session.sql("ls @wikipedia_mountain_assets").collect()
list_docs = []
for doc in docs:
    list_docs.append(doc["name"])
st.dataframe(list_docs, column_config={1:"Name"})

# Select Snowflake avialable models
